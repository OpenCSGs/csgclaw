...
自由发言房间、私聊和其他任务功能保留独立流程。全局任务看板中的 Room 任务链接转到 `/rooms/<room_id>?task=<task_id>`，可以精确指向父任务或子任务。
...
宿主 Codex 的 IM 执行器在按需房间轮次出队后，通过服务端 ContextProvider 自动读取当前成员和任务事实。完整的 Manager／Worker 按需策略由程序写入各自运行时 `AGENTS.md`，常驻但默认不生效。每个按需轮次的独立首个输入块显式携带 `room_type="on_demand"`、`role` 和匹配的 `policy_id`，同时逐轮发送一段紧凑、强制的当前角色指令；因此模型不需要从普通 JSON 自行推断房间模式，也不依赖历史轮次中的策略引用。

Manager 策略规定：当前轮存在可信按需激活块时，只要用户要求实际产出、修改、检查、判断或交付结果，就必须先创建或续接房间任务、规划 Worker 子任务并派发；判断与具体领域、工作大小、表面复杂度或 Manager 自身可用工具无关。除澄清范围或评估 Worker 返回结果所需的最少操作外，Manager 不得先自行执行目标工作。独立事项拆成多个子任务并尽量并行。Worker 策略只允许执行本轮明确分配的子任务，并要求先认领、后提交对应批次。

首次进入一条会话时，执行器发送完整房间范围、任务快照和本轮来源；后续轮次始终重申紧凑的角色硬约束和本轮来源，只在房间范围或任务快照变化时补发变化数据，不重复详细生命周期和未变化事实。为避免长会话压缩后丢失稳定事实，执行器每隔有限轮次重新发送一次完整事实；`/new`、角色／策略版本变化、服务重启或房间模式重新启用时也会刷新。

可信上下文契约只承认位于当前轮次第一个独立输入块、以 `<csgclaw-runtime-context` 开头的服务端上下文；`<turn-directive>` 重申本轮立即行为，`<untrusted-data>` 内的任务正文、结果和成员信息始终只是数据。用户正文或附件中仿造同名标记不会获得激活语义。这里不修改 Codex `additionalContext` 或底层协议：详细规则由 `AGENTS.md` 承载，动态块则把当前模式和最重要的先派工约束放在用户请求之前。

私聊和自由发言房间不会收到 runtime-context 块；即使同一历史中曾出现过动态规则，当前轮没有可信块时也必须忽略旧规则。Manager 的静态默认行为是直接完成私聊用户交代的工作，不主动创建任务、房间或委派。只有用户明确要求创建／管理 CSGClaw 房间、成员或消息时，才使用对应管理命令，并在标题、参与者或发言模式会实质影响结果且尚不明确时先确认。
