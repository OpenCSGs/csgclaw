...
The UI calls these resources **Connectors**; Plugin packaging, CLI commands, and existing API paths remain unchanged.

Configure GitLab, Feishu, and llm-wiki connections once in **Resources > Connectors**.
...
Fill in the resource settings, choose **Test connection** to validate the current draft, and choose **Save configuration** when ready, then open an Agent's **Connectors** tab and choose **Add from resources**.
...
Feishu connectors own their App ID and App Secret independently of all Agent channels.
Every bound Agent uses the global connector identity; channel edits do not change connector credentials.
Testing uses the current draft and unchanged saved credentials without storing draft settings or credentials.
GitLab tests verify a supplied PAT with the instance's `/api/v4/user` before checking MCP connectivity and tool discovery.
The save API validates the candidate credentials, MCP initialization, and tool discovery before persisting new connection settings.
A failed validation keeps the existing resource and its live bindings unchanged; disabling or removing a resource remains possible when the upstream is unavailable.
Concurrent changes invalidate the checked draft and require a retry.
The connector form ends with a tool list that is collapsed by default.
Testing refreshes it for the current draft; changing the draft hides the previous list until retested.
...
For HTTP Feishu connectors, enter the MCP URL and the connector's own App ID/App Secret.
...
Connector credential changes replace its connections and token caches; disconnect, disable, and removal retain their existing lifecycle behavior.
...
| Endpoint | Purpose |
|---|---|
| `GET /api/v1/connectors/catalog` | Built-in definitions |
| `GET /api/v1/connectors/catalog/{app_id}` | Definition and connection defaults |
| `GET/POST /api/v1/connectors/resources` | List or create global resources |
| `GET/PATCH/DELETE /api/v1/connectors/resources/{resource_id}` | Manage a resource and inspect affected Agents |
| `POST /api/v1/connectors/resources:probe` | Test a resource using explicit credentials |
| `GET/POST /api/v1/agents/{agent_id}/connectors` | List or add resource bindings |
| `GET/PATCH/DELETE /api/v1/agents/{agent_id}/connectors/{installation_id}` | Read, enable/disable, or remove one binding |
| `POST /api/v1/agents/{agent_id}/connectors:probe` | Test with the Agent's resolved identity |
...
GitLab prefills `https://u-agentichub-gitlab-mcp-1qq.public.opencsg.com/mcp`; Feishu prefills `https://u-agentichub-lark-mcp-passthrough-1qr.public.opencsg.com/mcp`.
Each uses the matching OpenCSG login environment.
...
The chat composer keeps attachment upload only; legacy GitHub/GitLab connector controls and their frontend OAuth polling are removed.
