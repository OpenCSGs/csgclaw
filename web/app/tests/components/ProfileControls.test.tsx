...
describe("ProfileControls", () => {
  ...
  it("renders runtime-provided DSH permission modes without Codex-specific help", async () => {
          ...
          schemas={[
            {
              ...
              choices: [
                ...
                { value: "read-only", label_zh: "仅可查看", description_zh: "不能修改本地文件。" },
              ],
    ...
    expect(screen.getByRole("combobox", { name: "运行模式" })).toHaveTextContent("工作区内修改");
    ...
    await user.click(screen.getByRole("option", { name: "仅可查看" }));
    expect(onDraftChange).toHaveBeenCalledWith(
      expect.objectContaining({ runtime_options: { permission_mode: "read-only" } }),
    );
  });
