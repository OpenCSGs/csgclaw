...
export function AppSettingsDialog({
  ...
  const [form, setForm] = useState<AppForm>(() => initialAppForm(definition, existing, hasFeishuChannel));
    ...
    >
      <DialogContent className={styles.settingsDialog} portalContainer={portalContainer}>
              ...
              <>
                {feishu ? (
                  <Field
                    ...
                    hint={config.credential_source === "feishu_channel" ? t("appChannelReferenceHint") : undefined}
                  >
                    <Select
                      value={config.credential_source || "manual"}
                      triggerProps={{ "aria-label": t("appCredentialSource") }}
                      options={[
                        { value: "feishu_channel", label: t("appUseFeishuChannel") },
                        { value: "manual", label: t("appManualCredentials") },
                      ]}
                      onValueChange={(value) =>
                        updateConfig({ credential_source: value === "feishu_channel" ? "feishu_channel" : "manual" })
                      }
                    />
                  </Field>
                ) : null}
                {channelCredentials && !hasFeishuChannel ? (
        ...
        <DialogFooter className={styles.footer}>
          ...
          <Button
            disabled={Boolean(busy) || oauthUnsupported || !(stdio ? config.command?.trim() : config.url?.trim())}
            loading={busy === "probe"}
          ...
          <Button
            variant="primary"
            disabled={Boolean(busy) || !form.name.trim() || !tested || oauthUnsupported}
            loading={busy === "connect"}
            onClick={() => void execute("connect")}
          >
            {existing ? t("appSaveAndConnect") : t("appAddAndConnect")}
          </Button>
        </DialogFooter>
      </DialogContent>
    </DialogRoot>
