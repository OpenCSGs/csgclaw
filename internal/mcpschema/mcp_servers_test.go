...
func TestNormalizeMCPServersRejectsInvalidHTTPHeaderName(t *testing.T) {
	_, err := NormalizeMCPServers(map[string]any{
	...
	if got, want := errString(err), "mcpServers.remote.headers contains an invalid HTTP header name"; got != want {
		t.Fatalf("NormalizeMCPServers() error = %q, want %q", got, want)
	}
}
