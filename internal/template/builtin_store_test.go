...
const (
	testBuiltinOpenClawImage     = "opencsg-registry.cn-beijing.cr.aliyuncs.com/opencsghq/openclaw:20260702.25-csgclaw"
	testBuiltinCodexSandboxImage = "opencsg-registry.cn-beijing.cr.aliyuncs.com/opencsghq/csgclaw-codex-sandbox:20260701"
)
...
func TestBuiltinStoreListGetAndFetchWorkspace(t *testing.T) {
	...
	if got, want := items[0].ID, "codex-sandbox-worker"; got != want {
		t.Fatalf("List()[0].ID = %q, want %q", got, want)
	}
	if got, want := items[1].ID, "openclaw-manager"; got != want {
		t.Fatalf("List()[1].ID = %q, want %q", got, want)
	}
	if got, want := items[2].ID, "openclaw-worker"; got != want {
		t.Fatalf("List()[2].ID = %q, want %q", got, want)
	}
	if got, want := items[3].ID, "picoclaw-manager"; got != want {
		t.Fatalf("List()[3].ID = %q, want %q", got, want)
	}
	if got, want := items[4].ID, "picoclaw-worker"; got != want {
	...
	codexItem, err := store.Get(context.Background(), "codex-sandbox-worker")
	if err != nil {
		t.Fatalf("Get(codex-sandbox-worker) error = %v", err)
	}
	if got, want := codexItem.RuntimeKind, runtime.KindCodexSandbox; got != want {
		t.Fatalf("Get(codex-sandbox-worker).RuntimeKind = %q, want %q", got, want)
	}
	if got, want := codexItem.Image, testBuiltinCodexSandboxImage; got != want {
		t.Fatalf("Get(codex-sandbox-worker).Image = %q, want %q", got, want)
	}
...
func TestServiceListAggregatesBuiltinAndLocalWithDefaultStoreFactory(t *testing.T) {
	...
	if got, want := items[0].ID, "builtin.codex-sandbox-worker"; got != want {
		t.Fatalf("List()[0].ID = %q, want %q", got, want)
	}
	if got, want := items[1].ID, "builtin.openclaw-manager"; got != want {
		t.Fatalf("List()[1].ID = %q, want %q", got, want)
	}
	if got, want := items[2].ID, "builtin.openclaw-worker"; got != want {
		t.Fatalf("List()[2].ID = %q, want %q", got, want)
	}
	if got, want := items[3].ID, "builtin.picoclaw-manager"; got != want {
		t.Fatalf("List()[3].ID = %q, want %q", got, want)
	}
	if got, want := items[4].ID, "builtin.picoclaw-worker"; got != want {
		t.Fatalf("List()[4].ID = %q, want %q", got, want)
	}
	if got, want := items[5].ID, "local.team-helper"; got != want {
		t.Fatalf("List()[5].ID = %q, want %q", got, want)
	}
}
