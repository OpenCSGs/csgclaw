...
const (
	...
	csgclawPermissionsBeginMarker   = "# BEGIN csgclaw-managed permissions (do not edit; regenerated by runtime)"
	csgclawPermissionsEndMarker     = "# END csgclaw-managed permissions"
	csgclawExecutionBeginMarker     = "# BEGIN csgclaw-managed execution features (do not edit; regenerated by runtime)"
	csgclawExecutionEndMarker       = "# END csgclaw-managed execution features"
	csgclawProjectTrustBeginMarker  = "# BEGIN csgclaw-managed project trust (do not edit; regenerated by runtime)"
	csgclawProjectTrustEndMarker    = "# END csgclaw-managed project trust"
	csgclawWinSandboxBeginMarker    = "# BEGIN csgclaw-managed windows-sandbox (do not edit; regenerated by runtime)"
	csgclawWinSandboxEndMarker      = "# END csgclaw-managed windows-sandbox"
	...
	defaultRemoteMCPStartupTimeout  = 30
	memoryMinRolloutIdleHours       = 1
	readOnlyPermissionsProfile      = "csgclaw-read-only"
)
...
var (
	...
	rootDottedMemoriesMinIdleRe  = regexp.MustCompile(`^\s*memories\s*\.\s*min_rollout_idle_hours\s*=`)
	memoriesTableGenerateRe      = regexp.MustCompile(`^\s*generate_memories\s*=`)
	memoriesTableUseRe           = regexp.MustCompile(`^\s*use_memories\s*=`)
	memoriesTableMinIdleRe       = regexp.MustCompile(`^\s*min_rollout_idle_hours\s*=`)
	rootDottedWindowsSandboxRe   = regexp.MustCompile(`^\s*windows\s*\.\s*sandbox\s*=`)
	windowsTableHeaderRe         = regexp.MustCompile(`^\s*\[\s*windows\s*\]\s*(?:#.*)?$`)
	windowsTableSandboxRe        = regexp.MustCompile(`^\s*sandbox\s*=`)
)

var executionFeatureKeys = map[string]struct{}{
	"apps":                         {},
	"browser_use":                  {},
...
func buildSandboxConfigBlock(executionMode string) string {
	var b strings.Builder
...
func buildPermissionsConfigBlock(executionMode string) string {
	if executionMode != ExecutionModeReadOnly {
		return ""
	}
	var b strings.Builder
	b.WriteString(csgclawPermissionsBeginMarker)
	b.WriteString("\n")
	fmt.Fprintf(&b, "default_permissions = %s\n", strconv.Quote(readOnlyPermissionsProfile))
	fmt.Fprintf(&b, "permissions.%s.description = %s\n", readOnlyPermissionsProfile, strconv.Quote("CSGClaw read-only mode without local filesystem access."))
	fmt.Fprintf(&b, "permissions.%s.filesystem.%s = %s\n", readOnlyPermissionsProfile, tomlQuotedKey(":root"), strconv.Quote("deny"))
	b.WriteString(csgclawPermissionsEndMarker)
	b.WriteString("\n")
	return b.String()
}

func buildExecutionFeaturesConfigBlock(inFeaturesTable bool, executionMode string) string {
	var b strings.Builder
	...
	b.WriteString(csgclawExecutionEndMarker)
	b.WriteString("\n")
	return b.String()
}

func buildWindowsSandboxConfigBlock() string {
	var b strings.Builder
	b.WriteString(csgclawWinSandboxBeginMarker)
	b.WriteString("\n")
	// The elevated Codex sandbox provisions a dedicated Windows account and
	// triggers UAC. CSGClaw agents use the restricted-token implementation so
	// autonomous tool calls never repeatedly interrupt the desktop session.
	b.WriteString(`sandbox = "unelevated"` + "\n")
	b.WriteString(csgclawWinSandboxEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
func buildMemoryConfigBlock(inMemoriesTable, enabled bool) string {
	...
	if inMemoriesTable {
		...
		fmt.Fprintf(&b, "min_rollout_idle_hours = %d\n", memoryMinRolloutIdleHours)
	} else {
		fmt.Fprintf(&b, "memories.generate_memories = %t\n", enabled)
		fmt.Fprintf(&b, "memories.use_memories = %t\n", enabled)
		fmt.Fprintf(&b, "memories.min_rollout_idle_hours = %d\n", memoryMinRolloutIdleHours)
	}
...
func buildMCPServersBlock(mcpServers map[string]any, workspaceDir string) (string, error) {
	return buildMCPServersBlockForExecutionMode(mcpServers, workspaceDir, ExecutionModeStandard)
}

func buildMCPServersBlockForExecutionMode(mcpServers map[string]any, workspaceDir, executionMode string) (string, error) {
	...
	for idx, name := range names {
		...
		if executionMode == ExecutionModeReadOnly && mcpTrimmedString(entry["url"]) == "" {
			continue
		}
		if idx > 0 && b.Len() > len(csgclawMCPBeginMarker)+1 {
			b.WriteString("\n")
		}
		...
		if url := mcpTrimmedString(entry["url"]); url != "" {
			...
			if executionMode == ExecutionModeReadOnly {
				...
				delete(entry, "default_tools_approval_mode")
			}
			writeCodexMCPCommonFields(&b, entry)
			if executionMode == ExecutionModeReadOnly {
				b.WriteString(`default_tools_approval_mode = "writes"` + "\n")
			}
			writeCodexRemoteMCPDefaults(&b, entry)
			continue
		}
		command := mcpTrimmedString(entry["command"])
		if command == "" {
			return "", fmt.Errorf("%s.%s must declare command or url", mcpschema.MCPServersKey, name)
		}
...
func configureCodexHomeConfig(existing string, profile agentruntime.Profile, mcpServers map[string]any) string {
	return configureCodexHomeConfigWithWorkspaceForExecutionMode(existing, profile, mcpServers, "", ExecutionModeStandard)
}
...
func configureCodexHomeConfigWithWorkspace(existing string, profile agentruntime.Profile, mcpServers map[string]any, workspaceDir string) string {
	return configureCodexHomeConfigWithWorkspaceForExecutionMode(existing, profile, mcpServers, workspaceDir, ExecutionModeStandard)
}
...
func configureCodexHomeConfigWithWorkspaceForPlatform(
	...
	return configureCodexHomeConfigWithWorkspaceForPlatformAndExecutionMode(existing, profile, mcpServers, workspaceDir, isWindows, ExecutionModeStandard)
}

func configureCodexHomeConfigWithWorkspaceForPlatformAndExecutionMode(
	existing string,
	profile agentruntime.Profile,
	mcpServers map[string]any,
	workspaceDir string,
...
func configureCodexHomeConfigWithWorkspaceForPlatformAndRuntimeOptions(
	...
	content = stripManagedBlock(content, csgclawPermissionsBeginMarker, csgclawPermissionsEndMarker)
	content = stripManagedBlock(content, csgclawExecutionBeginMarker, csgclawExecutionEndMarker)
	content = stripManagedBlock(content, csgclawProjectTrustBeginMarker, csgclawProjectTrustEndMarker)
	content = stripManagedBlock(content, csgclawWinSandboxBeginMarker, csgclawWinSandboxEndMarker)
	...
	content = stripUserExecutionDirectives(content)
	if block := buildPermissionsConfigBlock(executionMode); block != "" {
		content = hoistManagedBlock(content, block)
	}

	if block := buildExecutionFeaturesConfigBlock(hasRootFeaturesTable(content), executionMode); block != "" {
	...
	content = hoistManagedBlock(content, buildSandboxConfigBlock(executionMode))
	if isWindows {
		windowsSandboxBlock := buildWindowsSandboxConfigBlock()
		if hasRootWindowsTable(content) {
			content = injectManagedBlockAfterHeader(content, windowsTableHeaderRe, windowsSandboxBlock)
		} else {
...
func writeCodexRemoteMCPDefaults(b *strings.Builder, entry map[string]any) {
	if _, ok := mcpInteger(entry["startup_timeout_sec"]); !ok {
		fmt.Fprintf(b, "startup_timeout_sec = %d\n", defaultRemoteMCPStartupTimeout)
	}
}
...
func insertManagedBlockBeforeFirstTable(content, block string) string {
	block = strings.TrimRight(block, "\n") + "\n"
	...
	for i, line := range lines {
		...
		after := strings.TrimLeft(strings.Join(lines[i:], "\n"), "\n")
		if before == "" {
			return block + "\n" + after
		}
		return before + "\n\n" + block + "\n" + after
...
func hasRootWindowsTable(content string) bool {
	for _, line := range strings.Split(content, "\n") {
		if windowsTableHeaderRe.MatchString(line) {
			return true
		}
	}
	return false
...
func stripLegacySandboxDirectives(content string, isWindows bool) string {
	lines := strings.Split(content, "\n")
	out := make([]string, 0, len(lines))
	...
	for _, line := range lines {
		...
		if strings.HasPrefix(trimmed, "[") {
			...
			inWindowsTable = windowsTableHeaderRe.MatchString(line)
		...
		if strings.HasPrefix(trimmed, "sandbox_mode") ||
			strings.HasPrefix(trimmed, "sandbox_workspace_write.") ||
			(isWindows && (rootDottedWindowsSandboxRe.MatchString(trimmed) ||
				(inWindowsTable && windowsTableSandboxRe.MatchString(trimmed)))) {
			continue
		}
		out = append(out, line)
	}
...
func stripUserExecutionDirectives(content string) string {
	lines := strings.Split(content, "\n")
	out := make([]string, 0, len(lines))
	currentTable := ""
	for _, line := range lines {
		...
		switch currentTable {
		case "":
			if key == "approval_policy" || key == "default_permissions" || key == "web_search" {
				continue
			}
			if featureKey, ok := strings.CutPrefix(key, "features."); ok {
				if _, managed := executionFeatureKeys[strings.TrimSpace(featureKey)]; managed {
					continue
				}
			}
		case "[features]":
			if _, managed := executionFeatureKeys[key]; managed {
				continue
			}
		}
		out = append(out, line)
	...
	for _, line := range lines {
		...
		case "":
			if rootDottedFeaturesMemoriesRe.MatchString(trimmed) ||
				...
				rootDottedMemoriesMinIdleRe.MatchString(trimmed) {
				continue
			}
		case "[features]":
		...
		case "[memories]":
			if memoriesTableGenerateRe.MatchString(trimmed) ||
				...
				memoriesTableMinIdleRe.MatchString(trimmed) {
				continue
			}
		}
		out = append(out, line)
