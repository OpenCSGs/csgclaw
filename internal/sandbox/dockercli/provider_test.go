...
func TestCreateBuildsRunCLIArgs(t *testing.T) {
	runner := &fakeRunner{
		results: []fakeResult{{result: CommandResult{Stdout: []byte("container-id\n")}}},
	}
	rt, err := NewProvider(
		WithPath("/usr/local/bin/docker"),
		WithRunner(runner),
	).Open(context.Background(), "/tmp/ignored")
	if err != nil {
	...
	if got, want := runner.requests[0].Path, "/usr/local/bin/docker"; got != want {
...
func TestInstanceMethodsBuildCLIArgs(t *testing.T) {
	runner := &fakeRunner{
		results: []fakeResult{
			...
			{result: CommandResult{}},
			{result: CommandResult{Stdout: []byte(`[{"Id":"box-id","Name":"/agent","Created":"2026-04-18T07:31:25.471080+00:00","State":{"Status":"running"}}]`)}},
			{result: CommandResult{}},
		},
	}
...
func TestStopWithTimeoutUsesFlag(t *testing.T) {
	runner := &fakeRunner{results: []fakeResult{{result: CommandResult{}}}}
	rt, err := NewProvider(WithRunner(runner)).Open(context.Background(), "/tmp/x")
	if err != nil {
		t.Fatalf("Open() error = %v", err)
	}
	inst := &Instance{runtime: rt.(*Runtime), idOrName: "c1"}
	if err := inst.Stop(context.Background(), sandbox.StopOptions{Timeout: 1500 * time.Millisecond}); err != nil {
		t.Fatalf("Stop() error = %v", err)
	}
	want := []string{"stop", "-t", "2", "c1"}
	if got := runner.requests[0].Args; !reflect.DeepEqual(got, want) {
		t.Fatalf("args = %#v, want %#v", got, want)
	}
}

func TestStopForceUsesKill(t *testing.T) {
	runner := &fakeRunner{results: []fakeResult{{result: CommandResult{}}}}
	rt, err := NewProvider(WithRunner(runner)).Open(context.Background(), "/tmp/x")
	if err != nil {
		t.Fatalf("Open() error = %v", err)
	...
	inst := &Instance{runtime: rt.(*Runtime), idOrName: "c1"}
	if err := inst.Stop(context.Background(), sandbox.StopOptions{Force: true}); err != nil {
		t.Fatalf("Stop() error = %v", err)
	}
	want := []string{"kill", "c1"}
	if got := runner.requests[0].Args; !reflect.DeepEqual(got, want) {
...
func TestNotFoundErrorsMapToSandboxNotFound(t *testing.T) {
	runner := &fakeRunner{
		results: []fakeResult{{
			result: CommandResult{Stderr: []byte("Error: No such container: missing\n"), ExitCode: 1},
			err:    &exec.ExitError{},
		}},
	}
	rt, err := NewProvider(WithRunner(runner)).Open(context.Background(), "/tmp/x")
...
func TestIntegrationCreateStartExecRemove(t *testing.T) {
	if os.Getenv("CSGCLAW_DOCKER_CLI_INTEGRATION") != "1" {
		t.Skip("set CSGCLAW_DOCKER_CLI_INTEGRATION=1 to run docker CLI integration test")
	}
	path := strings.TrimSpace(os.Getenv("CSGCLAW_DOCKER_CLI_PATH"))
	if path == "" {
		path = "docker"
	}
