...
func (s *Controller) EffectiveAgentProfileForUpdate(id string, req UpdateRequest) (AgentProfile, error) {
...
func (s *Controller) UpdateAgentProfile(id string, profile AgentProfile) (AgentProfileView, error) {
	id = strings.TrimSpace(id)
	if id == "" {
...
func (s *Controller) syncGatewayAfterProfileChange(ctx context.Context, id string, previous Agent, normalized AgentProfile, restartRequired bool) error {
...
func (s *Controller) syncGatewayHostConfig(ctx context.Context, got Agent, profile AgentProfile) error {
	rt, err := s.runtimeForKind(got.RuntimeKind)
	if err != nil {
		return err
	}
	got.AgentProfile = profile
	if reconciler, ok := rt.(agentruntime.HostConfigReconciler); ok {
		gateway, err := s.gatewayProvisionRequest(got.RuntimeKind, got.Name, got.ID)
		if err != nil {
			return err
		}
		servers, err := s.materializeRuntimeMCPServers(ctx, got.RuntimeKind, got.MCPServers)
		if err != nil {
			return err
		}
		if err := reconciler.ReconcileHostConfig(ctx, agentruntime.ProvisionRequest{AgentID: got.ID, AgentName: got.Name, ParticipantID: participantIDForAgent(got.Name, got.ID), Profile: s.runtimeProfileForAgent(got), MCPServers: servers, Gateway: gateway}); err != nil {
			return err
		}
	}
	s.mu.Lock()
	...
	s.putAgentLocked(got.ID, current)
	return s.saveLocked()
}

func (s *Controller) UpdateRecord(ctx context.Context, id string, req UpdateRequest) (Agent, error) {
	if s == nil {
...
func (s *Controller) update(ctx context.Context, id string, req UpdateRequest) (Agent, error) {
	return s.updateWithManagedRuntimeOptions(ctx, id, req, false)
}

func (s *Controller) updateWithManagedRuntimeOptions(ctx context.Context, id string, req UpdateRequest, allowManagerRuntimeOptions bool) (Agent, error) {
	id = strings.TrimSpace(id)
	if id == "" {
...
func (s *Controller) AddMCPServersFromHub(ctx context.Context, id string, names []string, catalogServers map[string]any) (Agent, error) {
	id = strings.TrimSpace(id)
	if id == "" {
		return Agent{}, fmt.Errorf("agent id is required")
	}
...
func (s *Controller) DeleteMCPServers(ctx context.Context, id string, names []string) (Agent, error) {
	id = strings.TrimSpace(id)
	if id == "" {
		return Agent{}, fmt.Errorf("agent id is required")
	}
...
func (s *Controller) currentMCPServersForManagement(ctx context.Context, current Agent) (map[string]any, error) {
...
func (s *Controller) validateRuntimeConfig(ctx context.Context, runtimeKind string, current agentruntime.RuntimeConfigSnapshot) error {
	if s == nil {
...
func (s *Controller) validateRuntimeStartConfig(ctx context.Context, runtimeKind string, current agentruntime.RuntimeConfigSnapshot) error {
	if s == nil {
		return fmt.Errorf("agent service is required")
	}
	if err := s.checkRuntimeAvailability(ctx, runtimeKind); err != nil {
...
func (s *Controller) runtimeConfigRestartRequired(runtimeKind string, change agentruntime.RuntimeConfigChange) (bool, error) {
	if s == nil {
...
func (s *Controller) validateMCPServers(ctx context.Context, runtimeKind string, current agentruntime.MCPServersSnapshot) error {
	if s == nil {
		return fmt.Errorf("agent service is required")
	}
...
func (s *Controller) mcpServersRestartRequired(runtimeKind string, change agentruntime.MCPServersChange) (bool, error) {
	if s == nil {
		return false, fmt.Errorf("agent service is required")
	}
...
func (s *Controller) reconcileRuntimeConfig(ctx context.Context, previous, current Agent) error {
	if s == nil {
		return fmt.Errorf("agent service is required")
	}
	runtimeKind := strings.TrimSpace(current.RuntimeKind)
...
func (s *Controller) reconcileMCPServers(ctx context.Context, previous, current Agent) error {
	if s == nil {
		return fmt.Errorf("agent service is required")
	}
	runtimeKind := strings.TrimSpace(current.RuntimeKind)
...
func (s *Controller) Upgrade(ctx context.Context, id string) (Agent, error) {
	ctx, release, err := s.acquireAgentLifecycle(ctx, id)
	if err != nil {
		return Agent{}, err
	}
...
func (s *Controller) persistRecreatedAgent(ctx context.Context, id, image string, info agentruntime.Info) (Agent, error) {
	s.mu.Lock()
	current, key, ok := s.agentByIDLocked(id)
	if !ok {
		s.mu.Unlock()
	...
	delete(s.agents, key)
	s.putAgentLocked(current.ID, current)
...
func (s *Controller) profileForCreateRequest(ctx context.Context, spec *CreateAgentSpec) (AgentProfile, error) {
	if spec == nil {
