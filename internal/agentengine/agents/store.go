...
type persistedAgent struct {
	...
	DetectionResults   []ProfileDetectionResult   `json:"detection_results,omitempty"`
	RuntimeExtensions  map[string]json.RawMessage `json:"-"`
}
...
func (a persistedAgent) MarshalJSON() ([]byte, error) {
	...
	if len(a.RuntimeExtensions) > 0 {
		out["runtime_extensions"] = a.RuntimeExtensions
	}
...
func (a *persistedAgent) UnmarshalJSON(data []byte) error {
	...
	type persistedAgentJSON struct {
		...
		RuntimeExtensions  map[string]json.RawMessage `json:"runtime_extensions"`
	}
	...
	*a = persistedAgent(decoded.persistedAgentAlias)
	...
	a.RuntimeExtensions = cloneRawMessages(decoded.RuntimeExtensions)
...
func newPersistedAgent(a Agent) persistedAgent {
	...
	return persistedAgent{
		...
		RuntimeExtensions:  cloneRawMessages(a.RuntimeExtensions),
	}
}
...
// Reload replaces the snapshot only after the complete persisted state validates.
func (s *Repository) Reload() error {
	s.mu.Lock()
	defer s.mu.Unlock()
	candidate := &Repository{state: s.state, profileDefaults: cloneProfile(s.profileDefaults), normalizeProfileReference: s.normalizeProfileReference}
	agents, err := candidate.readState()
	if err != nil {
		return err
	}
	s.agents = agents
	s.runtimeRecords = candidate.runtimeRecords
	s.profileDefaults = candidate.profileDefaults
	s.detectionResults = candidate.detectionResults
	if s.runtimeRecords == nil {
		s.runtimeRecords = make(map[string]RuntimeRecord)
	}
	return nil
}

func (s *Repository) readState() (map[string]Agent, error) {
...
func (s *Repository) readRootAgentsState() (rootAgentsState, bool, error) {
...
func (s *Repository) agentsFromRootState(root rootAgentsState) (map[string]Agent, error) {
	agents := make(map[string]Agent)
...
func (s *Repository) saveLocked() error {
	if s.state == "" {
		return nil
	}
...
func (s *Repository) rootAgentsStateLocked() rootAgentsState {
...
func (s *Repository) shouldWriteRootAgentsState() bool {
...
func (s *Repository) stateLooksLikeRootState() bool {
	if strings.TrimSpace(s.state) == "" {
		return false
	}
	return rootSectionExists(s.state, "agents")
...
func (s *Repository) stateIsDefaultRootStatePath() bool {
...
func (s *Repository) normalizeLoadedAgent(a Agent) (Agent, error) {
	a = *cloneAgent(&a)
...
func (s *Repository) catalogReferenceForLoadedProfile(profile AgentProfile) AgentProfile {
	if s.normalizeProfileReference != nil {
		return s.normalizeProfileReference(profile)
	}
	return profile
}
