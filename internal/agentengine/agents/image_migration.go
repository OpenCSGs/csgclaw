...
func (s *Controller) withRuntimeImageMigrationStatus(ctx context.Context, a Agent) Agent {
	a = *cloneAgent(&a)
...
func (s *Controller) imageForRecreate(ctx context.Context, a Agent) string {
...
func (s *Controller) imageForUpgrade(ctx context.Context, a Agent) (string, bool) {
...
func (s *Controller) imageUpgradeCandidate(ctx context.Context, a Agent) (string, bool) {
	candidate, ok := s.currentDefaultImageForAgent(ctx, a)
	if !ok {
		return "", false
	}
...
func (s *Controller) localImageCandidates(ctx context.Context) []string {
	if s == nil {
		return nil
	}
	s.mu.RLock()
...
func (s *Controller) currentDefaultImageForAgent(ctx context.Context, a Agent) (defaultAgentImage, bool) {
	...
	if hubSvc != nil {
		if rt, err := s.runtimeForKind(a.RuntimeKind); err == nil {
			if provider, ok := rt.(agentruntime.DefaultImageProvider); ok {
				if image := strings.TrimSpace(provider.DefaultImage()); image != "" {
					return defaultAgentImage{image: image}, true
				}
			}
		}
	}
...
func (s *Controller) defaultManagerImageForRuntime(ctx context.Context, runtimeKind string) (defaultAgentImage, bool) {
	if s == nil {
		return defaultAgentImage{}, false
	}
