...
func TestStartApplyHelperIncludesConfigPath(t *testing.T) {
	...
	if err := StartApplyHelper(ApplyHelperOptions{ConfigPath: configPath, Channel: "beta", SwitchChannel: true}); err != nil {
	...
	wantArgs := []string{"--config", configPath, "upgrade", "--channel", "beta", "--switch-channel"}
