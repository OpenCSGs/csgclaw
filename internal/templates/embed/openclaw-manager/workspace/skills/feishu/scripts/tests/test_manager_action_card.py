...
class ManagerActionCardTest(unittest.TestCase):
    ...
    def test_worker_finalize_uses_bind_restart_status(self):
        ...
        self.assertEqual(payload["worker_recreate_policy"], "worker_recreated")
        self.assertEqual(payload["config"]["bot_bind"]["participant_id"], "web-dev")
        self.assertEqual(payload["recreate"], {})

    def test_worker_finalize_reports_bind_restart_skipped(self):
        originals = {
            "load_state": commands.load_state,
            "poll_until_success": commands.poll_until_success,
            "configure_csgclaw": commands.configure_csgclaw,
        ...
        payload = json.loads(stdout.getvalue())
        self.assertTrue(payload["bot_ensured"])
        self.assertEqual(payload["worker_recreate_policy"], "restart_skipped")
        self.assertEqual(payload["config"]["bot_bind"]["participant_id"], "new-worker")
        self.assertEqual(payload["recreate"], {})
