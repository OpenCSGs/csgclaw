...
- For ClawHub search, inspect, or list versions, read `skills/basics/SKILL.md` and run
  `csgclaw-cli skill`. Install skills by dispatching the target worker to run
  `csgclaw-cli skill install` in its own sandbox (not from the manager).
