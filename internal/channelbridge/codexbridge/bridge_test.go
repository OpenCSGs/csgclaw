...
func TestFeishuHTTPClientStreamEventsConvertsMessageBusEvents(t *testing.T) {
	t.Parallel()
	...
	client := &FeishuHTTPClient{
		...
		HTTPClient: &http.Client{
			Transport: roundTripFunc(func(req *http.Request) (*http.Response, error) {
				...
				if got, want := req.URL.Path, "/api/v1/channels/feishu/participants/agent-dev/events"; got != want {
					t.Fatalf("event stream path = %q, want %q", got, want)
				}
				if got, want := req.Header.Get("Authorization"), "Bearer secret"; got != want {
					t.Fatalf("authorization = %q, want %q", got, want)
				...
				return &http.Response{
					...
					Body: io.NopCloser(strings.NewReader(
						"event: message\n" +
							"data: {\"type\":\"message.created\",\"room_id\":\"oc_alpha\",\"mention_bot_id\":\"agent-dev\",\"message\":{\"id\":\"om_1\",\"content\":\"<at user_id=\\\"ou_dev\\\"></at> hi\",\"mentions\":[{\"id\":\"ou_dev\",\"name\":\"agent-dev\"}]}}\n\n",
					)),
				}, nil
			}),
		},
	...
	events, errs := client.StreamEvents(context.Background(), "agent-dev", "")
	got, ok := <-events
	if !ok {
		t.Fatal("StreamEvents() produced no event")
	}
	for err := range errs {
		if err != nil {
			t.Fatalf("StreamEvents() error = %v", err)
	...
	if got.MessageID != "om_1" || got.RoomID != "oc_alpha" || got.ChatType != "group" || got.Text != "hi" {
		t.Fatalf("event = %+v, want converted Feishu group event", got)
	}
	if !slices.Contains(got.Mentions, "agent-dev") || !slices.Contains(got.Mentions, "ou_dev") {
		t.Fatalf("mentions = %+v, want participant and open_id", got.Mentions)
	}
}

func TestFeishuHTTPClientSendMessageUsesFeishuRoute(t *testing.T) {
	t.Parallel()
	...
	client := &FeishuHTTPClient{
		...
		HTTPClient: &http.Client{
			Transport: roundTripFunc(func(req *http.Request) (*http.Response, error) {
				...
				if got, want := req.URL.Path, "/api/v1/channels/feishu/messages"; got != want {
					t.Fatalf("send message path = %q, want %q", got, want)
				}
				if got, want := req.Header.Get("Authorization"), "Bearer secret"; got != want {
					t.Fatalf("authorization = %q, want %q", got, want)
				...
				if payload["room_id"] != "oc_alpha" || payload["sender_id"] != "agent-dev" || payload["content"] != "hello" {
					t.Fatalf("payload = %#v, want Feishu message request", payload)
				}
				return &http.Response{
					StatusCode: http.StatusCreated,
					Header:     make(http.Header),
					Body:       io.NopCloser(strings.NewReader(`{"id":"om_reply"}`)),
				}, nil
			}),
		},
	}

	got, err := client.SendMessage(context.Background(), "agent-dev", SendMessageRequest{
		RoomID: "oc_alpha",
		Text:   "hello",
	})
	if err != nil {
	...
	if got.MessageID != "om_reply" {
		t.Fatalf("MessageID = %q, want om_reply", got.MessageID)
	}
}
