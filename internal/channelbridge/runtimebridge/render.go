...
func (r *TurnRenderer) FinalMessages() []string {
	...
	if r.promptError != "" {
		messages = append(messages, r.publicPromptError().Message)
	}
	return messages
}
...
func (r *TurnRenderer) SetLocale(locale string) {
	if r != nil {
		r.locale = strings.TrimSpace(locale)
	}
}

type PublicPromptError struct {
	Code    string
	Message string
}

var promptErrorHTTPStatusPattern = regexp.MustCompile(`(?i)\bunexpected\s+status\s+(\d{3})\b`)

func (r *TurnRenderer) PromptError() PublicPromptError {
	if r == nil || r.promptError == "" {
		return PublicPromptError{}
	}
	return r.publicPromptError()
}

func (r *TurnRenderer) publicPromptError() PublicPromptError {
	code := "internal_error"
	if status, ok := promptErrorHTTPStatus(r.promptError); ok {
		switch {
	...
	if billingURL := promptErrorBillingURL(r.promptError); code == "insufficient_balance" && billingURL != "" {
		label := "Recharge account"
		if zh {
			label = "前往充值"
		}
		message += fmt.Sprintf("\n\n👉 [%s](%s)", label, billingURL)
	}
	return PublicPromptError{Code: code, Message: message}
}

func promptErrorHTTPStatus(raw string) (int, bool) {
	match := promptErrorHTTPStatusPattern.FindStringSubmatch(raw)
	if len(match) != 2 {
		return 0, false
	}
...
func promptErrorBillingURL(raw string) string {
	marker := "[Recharge your account]("
...
// IsPromptErrorMessage reports whether text is the error message generated by
// this renderer, rather than ordinary model-authored output with similar text.
func (r *TurnRenderer) IsPromptErrorMessage(text string) bool {
	return r != nil && r.promptError != "" && strings.TrimSpace(text) == strings.TrimSpace(r.publicPromptError().Message)
}
