...
func (c BootstrapConfig) Resolved() BootstrapConfig {
	c.ManagerImage = strings.TrimSpace(c.ManagerImage)
	c.ManagerRootfsPath = strings.TrimSpace(c.ManagerRootfsPath)
	if c.ManagerImage == "" && c.ManagerRootfsPath == "" {
		c.ManagerImage = DefaultManagerImage
	}
	return c
}

func (c BootstrapConfig) Validate() error {
	if strings.TrimSpace(c.ManagerImage) != "" && strings.TrimSpace(c.ManagerRootfsPath) != "" {
		return fmt.Errorf("bootstrap manager_image and manager_rootfs_path are mutually exclusive")
	}
	if strings.TrimSpace(c.ManagerImage) == "" && strings.TrimSpace(c.ManagerRootfsPath) == "" {
		return fmt.Errorf("bootstrap manager source is required")
	}
	return nil
}

func (c BootstrapConfig) DisplaySource() string {
	c = c.Resolved()
	if c.ManagerRootfsPath != "" {
		return fmt.Sprintf("rootfs path %q", c.ManagerRootfsPath)
	}
	return fmt.Sprintf("image %q", c.ManagerImage)
}
...
func Load(path string) (Config, error) {
	...
	for scanner.Scan() {
		...
		case section == "bootstrap":
			...
			case "manager_rootfs_path":
				cfg.Bootstrap.ManagerRootfsPath = value
			}
...
[bootstrap]
`, cfg.Server.ListenAddr, cfg.Server.AdvertiseBaseURL, cfg.Server.AccessToken)
	if bootstrap.ManagerImage != "" {
		fmt.Fprintf(&b, "manager_image = %q\n", bootstrap.ManagerImage)
	}
	if bootstrap.ManagerRootfsPath != "" {
		fmt.Fprintf(&b, "manager_rootfs_path = %q\n", bootstrap.ManagerRootfsPath)
	}
	fmt.Fprintf(&b, `
...
[models]
default = %q
`, cfg.Sandbox.Resolved().Provider, cfg.Sandbox.Resolved().HomeDirName, cfg.Sandbox.Resolved().BoxLiteCLIPath, defaultSelector)
