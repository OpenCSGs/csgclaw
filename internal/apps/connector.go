...
func normalizeConfig(c Config, appID string) Config {
	if c.Transport == "" {
		if c.Command != "" {
			c.Transport = "stdio"
		} else {
...
func (s *Service) resolve(ctx context.Context, agentID string, c Config, credentials Credentials) (Credentials, error) {
	...
	if c.AuthMode != "none" && c.AuthMode != "feishu" && credentials.Token == "" {
...
func buildTransport(c Config, credentials Credentials, dirs processDirs) (mcp.Transport, error) {
	...
	client, err := connectorHTTPClient(c, credentials)
	if err != nil {
		return nil, err
	}
...
func connectorHTTPClient(c Config, credentials Credentials) (*http.Client, error) {
	headers := http.Header{}
	...
	case "feishu":
		if c.AppIDHeader == "" || c.AppSecretHeader == "" {
			return nil, fmt.Errorf("%w: specify the MCP service's App ID and App Secret headers", ErrInvalid)
		}
		if err := set(c.AppIDHeader, credentials.AppID); err != nil {
			return nil, err
		}
		if err := set(c.AppSecretHeader, credentials.AppSecret); err != nil {
			return nil, err
		}
	}
	u, _ := url.Parse(c.URL)
	client := &http.Client{Transport: &headerTransport{base: http.DefaultTransport, headers: headers, origin: u.Scheme + "://" + u.Host}, CheckRedirect: func(*http.Request, []*http.Request) error { return http.ErrUseLastResponse }}
	return client, nil
}
...
func (s *Service) open(ctx context.Context, agentID string, c Config, credentials Credentials, dirs processDirs, onToolsChanged func()) (*connection, *ProbeResult, error) {
	...
	transport, err := buildTransport(c, resolved, dirs)
	if err != nil {
		return nil, nil, err
	}
	...
	conn := &connection{ctx: lifetime, session: session, cancel: cancel, refreshMu: make(chan struct{}, 1)}
