...
const (
	...
	InstallStageNode        = "installing_node"
...
var (
	...
	ErrNodeInstallFailed  = errors.New("managed Node.js installation failed")
...
type Installer struct {
	...
	HTTPClient         HTTPDoer
	InstallManagedNode ManagedNodeInstaller
...
func (i Installer) InstallWithProgress(ctx context.Context, report ProgressReporter) (Info, error) {
	...
	nodeRuntime, err := i.resolveNodeRuntime(ctx, installRoot, report)
	if err != nil {
		return Info{}, err
	}
	...
	if err := createLauncher(actualPath, launcherPath, nodeRuntime.BinDir, goos); err != nil {
		return Info{}, err
	}
	reportProgress(report, InstallStageVerifying, 0)
	versionOut, err := i.run(ctx, launcherPath, "--version")
	if err != nil {
...
func (i Installer) runNPM(
	...
	cmd.Env = prependExecutablePath(os.Environ(), nodeBinDir, i.resolvedGOOS())
...
func prependExecutablePath(environment []string, directory, goos string) []string {
	...
	for index, entry := range out {
		...
		if found && strings.EqualFold(strings.TrimSpace(key), "PATH") {
			out[index] = key + "=" + directory + separator + value
			return out
		}
	}
	return append(out, "PATH="+directory)
}
...
const managedLauncherMarker = "Managed by CSGClaw for DeepSeek Harness"

func createLauncher(actualPath, launcherPath, nodeBinDir, goos string) error {
	...
	contents := managedLauncherContents(actualPath, nodeBinDir, goos)
	if info, err := os.Lstat(launcherPath); err == nil {
		if info.Mode()&os.ModeSymlink != 0 {
			...
			if readErr == nil && filepath.Clean(target) == filepath.Clean(actualPath) {
				...
				if removeErr := os.Remove(launcherPath); removeErr != nil {
					return fmt.Errorf("%w: replace managed launcher: %v", ErrInstallFailed, removeErr)
				}
			}
		} else if existing, readErr := os.ReadFile(launcherPath); readErr == nil && strings.Contains(string(existing), managedLauncherMarker) {
			if len(contents) == 0 {
				if removeErr := os.Remove(launcherPath); removeErr != nil {
					return fmt.Errorf("%w: replace managed launcher: %v", ErrInstallFailed, removeErr)
				}
				...
				if writeErr := os.WriteFile(launcherPath, contents, 0o755); writeErr != nil {
					return fmt.Errorf("%w: update managed launcher: %v", ErrInstallFailed, writeErr)
				}
				return nil
			}
		}
...
func managedLauncherContents(actualPath, nodeBinDir, goos string) []byte {
	nodeBinDir = strings.TrimSpace(nodeBinDir)
	if nodeBinDir == "" {
		return nil
	}
	if goos == "windows" {
		return []byte("@echo off\r\nrem " + managedLauncherMarker + "\r\nset \"PATH=" + nodeBinDir + ";%PATH%\"\r\ncall \"" + actualPath + "\" %*\r\n")
	}
	return []byte("#!/bin/sh\n# " + managedLauncherMarker + "\nPATH=" + shellQuote(nodeBinDir) + ":$PATH\nexport PATH\nexec " + shellQuote(actualPath) + " \"$@\"\n")
}
